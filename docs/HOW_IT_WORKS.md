...
The continuous loop requests up to 30 new references per later batch. Its budgets prioritize sources behind in the run's cumulative totals; when already balanced, each gets ten. Results from a leading source wait while the others catch up. If a source finishes or times out with too few matches, a notice explains the shortfall and queued results from the remaining sources are released. Missing references are never invented or replaced by extra results beyond another source's budget. A source gap can therefore still leave unequal totals.
