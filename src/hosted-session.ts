...
export function readCursor(value: unknown): { context: DiscoveryContext; emptyRounds: number } {
  ...
  return { context: { round: v.round + 1, target: 30, seenIds: new Set(v.seenIds), seenImages: new Set(v.seenImages), queries: Object.fromEntries(SOURCE_KEYS.map(key => [key, new Set(v.queries[key])])) as Record<SourceKey, Set<string>>, pages: new Map(v.pages) }, emptyRounds: v.emptyRounds };
}

export function saveCursor(context: DiscoveryContext, emptyRounds: number): DiscoveryCursor {
  return { round: context.round, emptyRounds, seenIds: [...context.seenIds], seenImages: [...context.seenImages], queries: Object.fromEntries(SOURCE_KEYS.map(key => [key, [...context.queries[key]]])) as Record<SourceKey, string[]>, pages: [...context.pages] };
}
