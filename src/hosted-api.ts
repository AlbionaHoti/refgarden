...
export async function handleResearch(request: Request, runBatch = runSourceSearch) {
  try {
    ...
    if (Object.keys(body).some(name => !['mode', 'continuous', 'brief', 'styles', 'selected', 'cursor'].includes(name))) throw new RequestError('Send only your prompt, styles and search history. API keys are not accepted.');
    ...
    const stream = new ReadableStream<Uint8Array>({
      start(controller: ReadableStreamDefaultController<Uint8Array>) {
        ...
        void (async () => {
          ...
          await runBatch({ ...input, styles }, signal, event => {
          ...
          if (empty >= 3) send({ type: 'end', atMs: 0, status: 'complete', message: 'These searches stopped returning new images. Edit your prompt to explore another direction.' });
